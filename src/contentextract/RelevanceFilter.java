...
public class RelevanceFilter {
	...
	protected void collectContentSocreThreshold()
	{
		...
		System.out.println("Content score threshold:"+MetricWeights.CONTENT_SCORE_THRESHOLD);
	}
	...
	protected Element filterDOMTree()
	{
		...
		for(int i=0;i<children.size();i++){
			...
			if(this.deleteTags.contains(child.tagName())){
				...
				try{
				double contscore=Double.parseDouble(child.attr("contscore"));
				//double contscore=Double.parseDouble(child.attr("scoresum"));
