...
public class DensityCalculator {
	...
	public DensityCalculator(Element element, String contextDesc, double bodyTextDensity, double bodyLinkDensity) {
	...
	public Element getDensities() {
		...
		this.element.attr("contdensity",String.format("%.2f", contentDensity));
	...
	protected double getTextDensity(Element element) {
		...
		int tagNumber = element.getAllElements().size()-1; //only the inner nodes
	...
	protected double getContentDensity(Element element)
	{
		...
		double nonLinkDensity=textDensity>linkDensity?textDensity-linkDensity:1;
		
		double logbase=getLogBase(textDensity, linkDensity,nonLinkDensity );
		double logPart=Math.log(textDensity/linkDensity+codeDensity/textDensity);
		logPart=logPart/Math.log(logbase);
		
		contentDensity=firstPart*logPart;
		if(contentDensity<0)contentDensity=0;
		return contentDensity;
	}
	
	
	protected double getLogBase(double textDensity, double linkDensity, double nonLinkDensity)
	{
		//code for getting the log base
		if(nonLinkDensity==0)nonLinkDensity=1;
		if(bodyTextDensity==0)bodyTextDensity=1;
		double logbase=Math.log((textDensity*linkDensity/nonLinkDensity)+(bodyLinkDensity*textDensity/bodyTextDensity)+e);
		return logbase;
	}
