...
public class MetricNormScoreCalculator {
	...
	protected void collectMaximumValues()
	{
		...
		for(Element elem:elems){
			try{
			double contentDensity=Double.parseDouble(elem.attr("contdensity"));
	...
	public Element normalizeAndCalcScoreSubTree(Element elem){
		...
		elem.attr("contdensity",String.format("%.2f", contDensity));
		elem.attr("contrel",String.format("%.2f", contRelevance));
		double contentScore=contDensity*MetricWeights.CONTENT_DENSITY_WEIGHT+
				contRelevance*MetricWeights.CONTENT_RELEVANCE_WEIGHT;
		elem.attr("contscore",String.format("%.2f", contentScore));
		}catch(Exception e){
