...
public class RelevanceCalculator {
	...
	public RelevanceCalculator(Element element, String contextdesc, String strace, String ccontext){
	...
	public Element getRelevance() {
		...
		contentRelevance=codeRelAvreage
				* MetricWeights.CODE_RELEVANCE_WEIGHT + textRelAverage
				* MetricWeights.TEXT_RELEVANCE_WEIGHT;
		element.attr("crel", String.format("%.2f", codeRelAvreage));
		element.attr("trel", String.format("%.2f", textRelAverage));
		element.attr("contrel", String.format("%.2f", contentRelevance));
		return element;
	}
	...
	protected double getCodeTagAvgRelevance(Element element) {
		...
		if(tagNumber==0)return 0;
		if(tagNumber==1)return total_relevance;
		else{
	...
	protected double getTextTagRelevance(Element element) {
		...
		try {
			...
			if(Double.isNaN(textMatching))textMatching=0;
		} catch (Exception exc) {
	...
	protected double getStackTraceMatching(String tstrace)
	{
		...
		CosineSimilarityMeasure cosmeasure=new CosineSimilarityMeasure(strace, tstrace);
