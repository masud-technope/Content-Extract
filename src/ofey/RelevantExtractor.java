...
public class RelevantExtractor {
	...
	public RelevantExtractor(Element body, double thresholdCTD) {
		// initialization
		this.body = body;
	...
	public Element provideRelevantSection() {
		...
		Element modifiedBody = refineDOMTree();
		double maxScore = 0;
		int maxChildIndex = 0;
		...
		for (int i = 0; i < children.size(); i++) {
			...
			//double score = Double.parseDouble(elem.attr("compdensity"));
			double score = Double.parseDouble(elem.attr("compdensum"));
			if (score > maxScore) {
				maxScore = score;
				maxChildIndex = i;
			}
		}
		this.filterChildIndices.clear();
		for (int i = 0; i < children.size(); i++) {
			if (i != maxChildIndex) {
				this.filterChildIndices.add(i);
			}
		}
